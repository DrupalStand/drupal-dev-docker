...
with Drupal 8.3.5. You can upgrade with `make drupal-upgrade` or wait until this
...
The ElasticSearch container is configured with default settings which can be
modified within the container itself (e.g.: `docker exec -it elasticsearch
/bin/bash`).

The Drupal contrib module [ElasticSearch
Connector](http://drupal.org/project/elasticsearch_connector) is available as
part of the composer installation process when the CMS container is spun up.
When configuring the cluster, use the container name instead of an address.
